...
int imageDeleteDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
    	...
    	rc = queryAsyncOperation(image, output->operationId, "Image_Delete_DM", vmapiContextP);
    } else {
        // Handle SMAPI return code and reason code
...
int imageDiskCreateDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
        ...
        rc = queryAsyncOperation(image, output->operationId, "Image_Disk_Create_DM", vmapiContextP);
    } else {
        // Handle SMAPI return code and reason code
...
int imageDiskDeleteDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
        ...
        rc = queryAsyncOperation(image, output->operationId, "Image_Disk_Delete_DM", vmapiContextP);
    } else {
        // Handle SMAPI return code and reason code
...
int imageDiskShare(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    printf("Adding disk in %s's directory entry to %s's active configuration... ", tgtImage, image);
...
int imageDiskShareDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    printf("Adding disk in %s's directory entry to %s's directory entry... ", tgtImage, image);
...
int imageLockDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (strlen(address) == 0) {
       ...
       printf("Locking virtual image's device %s... ", address);
    }
...
int imageLockQueryDM(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
        printAndLogSmapiCallReturnCode("Image_Lock_Query_DM", rc, vmapiContextP);
        ...
        if (strcmp(lockedType, "IMAGE") == 0) {
            ...
            if (token != NULL) {
                ...
                printf("ERROR: imageLockedBy is NULL\n");
                return 1;
            }
...
int imageMDISKLinkQuery(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
        printAndLogSmapiCallReturnCode("Image_MDISK_Link_Query", rc, vmapiContextP);
    } else if (output->common.returnCode || output->common.reasonCode) {
        // Handle SMAPI return code and reason code
        ...
        if (imageCount > 0) {
            ...
            for (i = 0; i < imageCount; i++) {
                ...
                if (token != NULL) {
                    ...
                    printf("ERROR: systemName is NULL\n");
                    rc =1;
                    break;
                }
        	    ...
        	    //printf("output->mdiskLinkList[%d]%s\n", i, output->mdiskLinkList[i]);
...
int imagePause(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (strcmp(pauseOrBegin, "PAUSE=YES") == 0) {
        printf("Halting processing on %s... ", image);
    ...
    if (rc) {
        printAndLogSmapiCallReturnCode("Image_Pause", rc, vmapiContextP);
    } else {
        // Handle SMAPI return code and reason code
...
int imageVolumeShare(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
    ...
    if (rc) {
        printAndLogSmapiCallReturnCode("Image_Volume_Share", rc, vmapiContextP);
    } else {
        // Handle SMAPI return code and reason code
        ...
        switch (option) {
            ...
            case 'h':
                printf("NAME\n"
                    ...
                    "  The following options are required:\n"
                    "    -T    TTarget image or authorization entry name.\n"
