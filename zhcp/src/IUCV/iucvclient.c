...
*  @return 0: successfully.
*/
int handle_upgrade(int sockfd, char* buffer)
{
    char src_path[BUFFER_SIZE], recv_buf[BUFFER_SIZE];
    int returncode;
    /* Send the upgrade files to server.*/
    syslog(LOG_INFO,"Target system is %s, Send the upgrade files to server.\n", buffer);
    // iucvserv
    recv(sockfd, recv_buf, SMALL_BUFFER_SIZE, 0);
    if((returncode = send_file_to_server(sockfd, FILE_PATH_IUCV_SERVER))!= 0)
    {
        printf("Upgrade failed! Failed to send iucvserv file");
        return returncode;
    }
    if(strcmp(buffer, UPGRADE_NEEDED_SYSTEMV)==0)
    {
        // iucvserd
        ...
        {
            printf("Upgrade failed! Failed to send iucvserd file");
            return returncode;
        }
    }
    if(strcmp(buffer, UPGRADE_NEEDED_SYSTEMD)==0)
    {
        // iucvserd
        ...
        if((returncode = send_file_to_server(sockfd, src_path))!= 0)
        {
            printf("Upgrade failed! Failed to send iucvserd.service file");
            return returncode;
        }
    }
    // iucvupgrade.sh
    ...
    if((returncode = send_file_to_server(sockfd, src_path))!= 0)
    {
        printf("Upgrade failed! Failed to send iucvupgrade.sh file");
        return returncode;
    }
