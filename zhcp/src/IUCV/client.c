...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    if(strcmp(buffer, FILE_TRANSPORT)==0)
    {
        syslog(LOG_INFO,"Will send file %s to IUCV server.\n", argv[2]);
        fp = fopen(argv[2], "r");
        if(NULL == fp)
        {
            syslog(LOG_ERR, "Failed to open file %s for read", argv[2]);
        }
        else
        {
            ...
            {
                sprintf(buffer,"Transport file %s successfully", argv[2]);
                printAndLogIUCVserverReturnCodeReasonCodeoutput(0, 0, buffer);
            }
            else
            {
                sprintf(buffer,"Transport file %s failed", argv[2]);
