...
int check_client_authorization(char *req_userid)
{
    ...
    * if the file exists, read the value in it and make comparation.*/
    if( NULL == fp)
    {
        syslog(LOG_ERR,"%s doesn't exist, create it userid len=%d\n", PATH_FOR_AUTHORIZED_USERID,strlen(req_userid));
...
int receive_file_from_client(int newsockfd, char *des_path)
{
    ...
    {
        buffer[n] = 0;
        syslog(LOG_INFO,"write n=%d,data=%s,len=%d\n",n, buffer, strlen(buffer));
        if(strncmp(buffer, FILE_SENT_OVER, strlen(FILE_SENT_OVER) )==0)
        {
            ...
            syslog(LOG_INFO,"file md5=%s",buffer);
            break;
        }
        ...
        syslog(LOG_INFO, "write to file a time",buffer);
        bzero(buffer,BUFFER_SIZE);
    }
    if (fclose(fp) != 0) 
    {
    ...
    */
    ...
    while(1)
    {
        ...
        */
        len = strcspn(buffer, " ");
        strncpy(tmp, buffer, len);
        syslog(LOG_INFO, "Current version is %s, upgraded version is %s", IUCV_SERVER_VERSION, tmp);
            ...
            {    
                ...
                while (fgets(buffer, sizeof(buffer), fp) != NULL)
                {
                    ...
                    */
                    ...
                    if(len >= BUFFER_SIZE)
                    {
                        syslog(LOG_INFO,"result len=%d strlen=%d,\n %s", len, strlen(send_buf), send_buf);
