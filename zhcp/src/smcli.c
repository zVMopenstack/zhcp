 ...
 */
int queryAsyncOperation(char* image, int operationId, char * apiName, struct _vmApiInternalContext *vmapiContextP) {
    ...
    while (true){
        ...
        if (rc || output->common.returnCode || output->common.reasonCode) {
            ...
            if (rc) {
                printAndLogSmapiCallReturnCode("Query_Asychronous_Operation_DM", rc, vmapiContextP);
                return rc;
            }    
